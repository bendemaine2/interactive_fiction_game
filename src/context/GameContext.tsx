...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const setPlayerType = useCallback((type: PlayerType) => {
    dispatch({ type: 'SET_PLAYER_TYPE', payload: type });
  }, []);
  ...
  const setWorldState = useCallback((ws: WorldState) => {
    dispatch({ type: 'SET_WORLD_STATE', payload: ws });
  }, []);

  const updateWackiness = useCallback((value: number) => {
    dispatch({ type: 'UPDATE_WACKINESS', payload: value });
  }, []);
  ...
  const appendProse = useCallback((entry: ProseEntry) => {
    dispatch({ type: 'APPEND_PROSE', payload: entry });
  }, []);

  const setFocusedCharacter = useCallback((id: string | null) => {
    dispatch({ type: 'SET_FOCUSED_CHARACTER', payload: id });
  }, []);
